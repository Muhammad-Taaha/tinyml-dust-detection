...
function StatusCard({ reading, status }) {
...
export default function DashboardScreen({ lastReading, status, onReconnect }) {
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Solar Panel Monitor</Text>
      <StatusCard reading={lastReading} status={status} />
...
const styles = StyleSheet.create({
  ...
  card:         { width: '100%', borderRadius: 16, borderWidth: 2, padding: 28,
                  alignItems: 'center', marginBottom: 20 },
  cardEmoji:    { fontSize: 56, marginBottom: 8 },
  cardLabel:    { fontSize: 32, fontWeight: '700', marginBottom: 16 },
  barBg:        { width: '100%', height: 10, backgroundColor: '#D3D1C7',
  ...
  reconnectBtn: { backgroundColor: '#534AB7', paddingVertical: 12, paddingHorizontal: 32,
                  borderRadius: 10, marginBottom: 20 },
  reconnectText:{ color: '#fff', fontWeight: '600', fontSize: 16 },
  details:      { width: '100%', backgroundColor: '#F1EFE8',
